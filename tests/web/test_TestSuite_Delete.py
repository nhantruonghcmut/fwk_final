...
class TestSignUpAndSignIn:
    ...
    def test_delete_account(self, browser, app_urls, data_test_delete, request):
        ...
        try:
            ...
            with allure.step("Save login session"):
                # Lưu session đăng nhập vào thư mục sessions/
                session_path = home_page.save_login_session(session_name=f"login_{data['test_id']}")
                allure.attach.file(session_path, name="Login Session", attachment_type=allure.attachment_type.JSON)
